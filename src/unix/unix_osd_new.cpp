...
static int       menu_focused_slot = 0;
...
static bool draw_menu(void)
{
    ...
    const bool up    = ImGui::IsKeyPressed(ImGuiKey_UpArrow,     false);
    const bool down  = ImGui::IsKeyPressed(ImGuiKey_DownArrow,   false);
    const bool home  = ImGui::IsKeyPressed(ImGuiKey_Home,        false);
    const bool end   = ImGui::IsKeyPressed(ImGuiKey_End,         false);
    ...
    if (tab)
        menu_focused_slot = (menu_focused_slot == 0) ? 1 : 0;

    if (menu_focused_slot == 1 && up)
        menu_focused_slot = 0;

    bool close_osd = false;
    if (menu_focused_slot == 0) {
        if (up) {
    ...
    /* Close button */
    ImGui::Separator();
    if (FocusedButton("Close OSD", menu_focused_slot == 1))
        close_osd = true;
...
static bool draw_log(void)
{
    ...
    if (log_focused_slot == 0 && !swallowed_up) {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_End))  ImGui::SetScrollHereY(1.0f);
    }
...
static bool draw_folder_browser(void)
{
    ...
    if (total == 0) {
        ...
        if (focused_slot == 0) {
            ...
            if (!swallowed_up && ImGui::IsKeyPressed(ImGuiKey_UpArrow)   && cd_folder_sel > 0)         cd_folder_sel--;
            if (ImGui::IsKeyPressed(ImGuiKey_DownArrow) && cd_folder_sel < total - 1)                  cd_folder_sel++;
            if (ImGui::IsKeyPressed(ImGuiKey_PageUp))   { int n = cd_folder_sel - 12; cd_folder_sel = n < 0       ? 0          : n; }
            if (ImGui::IsKeyPressed(ImGuiKey_PageDown)) { int n = cd_folder_sel + 12; cd_folder_sel = n >= total   ? total - 1  : n; }
            if (ImGui::IsKeyPressed(ImGuiKey_Home)) cd_folder_sel = 0;
            if (ImGui::IsKeyPressed(ImGuiKey_End))  cd_folder_sel = total > 0 ? total - 1 : 0;
...
static bool draw_file_selector(void)
{
    ...
    if (file_count == 0) {
        ...
        if (focused_slot == 0) {
            ...
            if (!swallowed_up && ImGui::IsKeyPressed(ImGuiKey_UpArrow)   && file_sel > 0)              file_sel--;
            if (ImGui::IsKeyPressed(ImGuiKey_DownArrow) && file_sel < file_count - 1)                  file_sel++;
            if (ImGui::IsKeyPressed(ImGuiKey_PageUp))  { int n = file_sel - 12; file_sel = n < 0            ? 0              : n; }
            if (ImGui::IsKeyPressed(ImGuiKey_PageDown)) { int n = file_sel + 12; file_sel = n >= file_count ? file_count - 1 : n; }
            if (ImGui::IsKeyPressed(ImGuiKey_Home))  file_sel = 0;
            if (ImGui::IsKeyPressed(ImGuiKey_End))   file_sel = file_count - 1;
